var TVjsApi = function(hashName) {

	this.symbol = hashName || "btcusdt";
	var thats = this;
...
Datafeeds.WebsockFeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
    var symbolName = "";
    if (thats.symbol == undefined || thats.symbol == '') {
        symbolName = "ethqc";
    } else {
        symbolName = thats.symbol.toLowerCase();
    }

    //symbolInfo配置很重要
    let data = {
            "symbol": thats.symbol.toLowerCase(),
            "has_daily": true,
            "ticker": thats.symbol.toLowerCase(),
            "name": thats.symbol.toLowerCase(),
            "description": thats.symbol.toLowerCase(),
            "type": "bitcoin",
            "exchange": "blackhorse.pro",
            "listed_exchange": "blackhorse.pro",
            ...
            "exchange-traded": thats.symbol.toLowerCase(),
            "minmove2": 0,
            "minmov": 1,
            "pricescale": 10000,
    ...
    $.get(this._datafeedURL + "/m/allticker/" + Date.parse(new Date()), function(res) {
        if (res.state == 1) {
            ...
            var ticker = res.data[thats.symbol.toLowerCase() + "_ticker"];
...
Datafeeds.WebsockFeed.prototype.getBars = function(symbolInfo, resolution, from, to, onHistoryCallback, onErrorCallback, firstDataRequest) {
    ...
    //根据时间周期来加载
    if (firstDataRequest || thats.symbol != oldthats.symbol) {
...
Datafeeds.WebsockFeed.prototype.subscribeBars = function(symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback) {
    ...
    function subscribeK() {
        ...
        $.get(that._datafeedURL + '/m/kline/' + thats.symbol + '/' + resolution + '/1', function(res) {
            var data = res.data.lines;
            if (data.length > 0) {
                var bars = data.map(el => {
                    return {
...
TVjsApi.prototype.init = function() {
	thats.symbol = this.symbol;
    thats.language = this.language;
this.widget = window.tvWidget = new TradingView.widget({
    ...
    width: "100%",
    height: "100%",
    container_id: "tv_chart_container_k",
    timezone: "Asia/Shanghai",
    // loading_screen: {
    ...
    overrides: {
        ...
        'paneProperties.background': '#191730', // 背景色
        ...
        'paneProperties.horzGridProperties.color': '#222', //横行线
        'paneProperties.vertGridProperties.color': '#222', //竖行线
        // 坐标轴和刻度标签颜色
        // 区域+字坐标颜色
...
tvWidget.onChartReady(function() {
    //均线
    tvWidget.chart().createStudy("Moving Average", true, false, [5], null, {
        "plot.color": "#b7248a"
    }, {
        checkLimit: false,
        priceScale: "as-series"
    });
    tvWidget.chart().createStudy("Moving Average", true, false, [10], null, {
        "plot.color": "#84aad5"
    }, {
        checkLimit: true,
        priceScale: "as-series"
    ...
    tvWidget.chart().createStudy("Moving Average", true, false, [60], null, {
        "plot.color": "#965fc4"
    }, {
        checkLimit: true,
        priceScale: "as-series"
    ...
    // 创建按钮
    tvWidget.createButton().attr('title', "1min").text("1min").on('click', function(e) {
        $(this).parents(".left").children().find(".button").removeAttr("style");
        staticResolutioin = 1;
        tvWidget.chart().setResolution('1', function onReadyCallback() {});
    });
    tvWidget.createButton().attr('title', "5min").text("5min").on('click', function(e) {
        $(this).parents(".left").children().find(".button").removeAttr("style");
        staticResolutioin = 5;
        tvWidget.chart().setResolution('5', function onReadyCallback() {});
    });
    tvWidget.createButton().attr('title', "15min").text("15min").on('click', function(e) {
        $(this).parents(".left").children().find(".button").removeAttr("style");
        staticResolutioin = 15;
        tvWidget.chart().setResolution('15', function onReadyCallback() {});
    });
    tvWidget.createButton().attr('title', "30min").text("30min").on('click', function(e) {
        $(this).parents(".left").children().find(".button").removeAttr("style");
        staticResolutioin = 30;
        tvWidget.chart().setResolution('30', function onReadyCallback() {});
    });
    tvWidget.createButton().attr('title', "1hour").text("1hour").on('click', function(e) {
        $(this).parents(".left").children().find(".button").removeAttr("style");
        staticResolutioin = 60;
        tvWidget.chart().setResolution('60', function onReadyCallback() {});
    });
    tvWidget.createButton().attr('title', "1day").text("1day").on('click', function(e) {
        $(this).parents(".left").children().find(".button").removeAttr("style");
        staticResolutioin = "D";
        tvWidget.chart().setResolution('D', function onReadyCallback() {});
    });
    tvWidget.createButton().attr('title', "1week").text("1week").on('click', function(e) {
        $(this).parents(".left").children().find(".button").removeAttr("style");
        staticResolutioin = "W";
        tvWidget.chart().setResolution('W', function onReadyCallback() {});
    });
    tvWidget.createButton().attr('title', "1mon").text("1mon").on('click', function(e) {
        $(this).parents(".left").children().find(".button").removeAttr("style");
        staticResolutioin = "M";
